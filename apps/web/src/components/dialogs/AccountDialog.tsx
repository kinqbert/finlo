...
export function AccountDialog(props: FinanceActions) {
  ...
  const form = useForm<AccountFormValues>({ resolver: zodResolver(accountSchema), defaultValues: { name: '', type: 'card', currency: 'UAH', balance: 0 } })
  const busy = form.formState.isSubmitting

  async function submit(values: AccountFormValues) {
    const timestamp = new Date().toISOString()
    const account: Account = { id: crypto.randomUUID(), name: values.name, type: values.type, currency: values.currency.toUpperCase(), balance_minor: Math.round(values.balance * 100), created_at: timestamp, updated_at: timestamp }
    setError('')
    try {
      ...
      <FinanceDialog busy={busy} eyebrow="New account" title="Connect your money map" description="Add a balance now. Monobank sync can be connected later.">
        <DialogForm onSubmit={form.handleSubmit(submit)} spaced>
          ...
          <div className="flex gap-3 max-[540px]:flex-col">
            ...
            <FormField label="Currency" error={form.formState.errors.currency?.message}><input maxLength={3} disabled={busy} {...form.register('currency')} /></FormField>
          </div>
          <FormField label="Current balance" error={form.formState.errors.balance?.message}><MoneyInput><input type="number" step="0.01" disabled={busy} {...form.register('balance', { valueAsNumber: true })} /></MoneyInput></FormField>
