...
function TransactionItem({ transaction, categories }: { transaction: Transaction; categories: Category[] }) {
  ...
  return (
    <MotionItem className="grid grid-cols-[auto_1fr_auto] items-center gap-2.5 border-b border-[#edf0e9] py-2 last:border-b-0" highlight>
      ...
      <div className="flex min-w-0 flex-col"><strong className="overflow-hidden text-[11px] text-ellipsis whitespace-nowrap">{transaction.description || transaction.category}</strong><div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1"><span className="text-[9px] text-muted">{transaction.category} · {new Intl.DateTimeFormat('en', { month: 'short', day: 'numeric' }).format(new Date(transaction.occurred_at))}</span>{transaction.category_needs_review && <CategoryAssignmentDialog transaction={transaction} categories={categories} />}</div></div>
