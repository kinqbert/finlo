...
function TransactionItem({ transaction }: { transaction: Transaction }) {
  ...
  return (
    <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2.5 border-b border-[#edf0e9] py-2 last:border-b-0">
  ...
  return (
    <div className="grid">
      ...
      {transactions.map((transaction) => (
        <div className="grid min-h-13 grid-cols-[1.5fr_1fr_.8fr_.7fr] items-center gap-3 border-t border-[#edf0e9] text-[10px] text-muted max-[540px]:grid-cols-[1.4fr_.8fr] max-[540px]:[&>span:nth-child(2)]:hidden max-[540px]:[&>span:nth-child(3)]:hidden" key={transaction.id}>
          <span className="flex items-center gap-2 text-ink"><span className="grid size-8 shrink-0 place-items-center rounded-[9px] bg-[#f2f3ed] text-[#496153]">{transaction.type === 'expense' ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />}</span><strong>{transaction.description || transaction.category}</strong></span>
