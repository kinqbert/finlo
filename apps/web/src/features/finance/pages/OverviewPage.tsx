  ...
  return <>
    ...
    <section className="mb-2.5 grid grid-cols-[1.6fr_repeat(3,1fr)] gap-2.5 max-[1100px]:grid-cols-[1.4fr_1fr_1fr] max-[820px]:grid-cols-2 max-[540px]:grid-cols-1">
      <div className="row-span-2 rounded-[15px] border border-brand bg-brand p-5 text-white shadow-[0_12px_26px_rgba(21,63,46,.14)] max-[820px]:col-span-2 max-[820px]:row-auto max-[540px]:col-auto">
        ...
        <strong className="my-5 block font-heading text-[clamp(28px,2.8vw,38px)] leading-none font-bold tracking-[-1.5px]">{hasAccounts ? formatMoney(primaryBalance.balance_minor, primaryBalance.currency) : 'No accounts yet'}</strong>
        ...
        <div className="mt-5 grid gap-2 border-t border-white/12 pt-3.5">{hasAccounts ? data.accounts.slice(0, 3).map((account, index) => <div className="grid grid-cols-[7px_1fr_auto] items-center gap-2 text-[10px]" key={account.id}><i className={`size-1.5 rounded-full ${index === 1 ? 'bg-[#ecba9e]' : index === 2 ? 'bg-[#a9a8e8]' : 'bg-lime'}`} /><span className="text-white/65">{account.name}</span><b>{formatMoney(account.balance_minor, account.currency, true)}</b></div>) : <button className="inline-flex cursor-pointer items-center gap-1 justify-self-start border-0 bg-transparent p-0 text-[10px] font-bold text-lime" onClick={() => onView('activity')}>Add your first account <ArrowUpRight size={13} /></button>}</div>
      </div>
    ...
    <section className="grid grid-cols-[1.45fr_.75fr] gap-2.5 max-[820px]:grid-cols-1">
      <Card fill><CardTitle title="Budget pace" action="View plans" onAction={() => onView('plans')} />{data.budgets.length ? <div className="grid gap-4">{data.budgets.map((budget) => <BudgetRow key={budget.id} budget={budget} />)}</div> : <EmptyState compact title="No budgets yet" copy="Set a monthly category limit to see your spending pace." action="Plan spending" onAction={() => onView('plans')} />}</Card>
      ...
      <Card fill className="min-h-64"><CardTitle title="Recent activity" action="See all" onAction={() => onView('activity')} /><TransactionList transactions={data.transactions.slice(0, 5)} /></Card>
      <Card fill className="min-h-64"><CardTitle title="Emergency fund" action="Manage" onAction={() => onView('plans')} /><Goal fund={data.dashboard.emergency_fund} /></Card>
    </section>
  </>
}
