...
export function OverviewPage({ user, ...actions }: FinanceActions & { user: User }) {
  ...
  const baseTotal = totalBalances.find((balance) => balance.currency === baseCurrencyCode) ?? data.dashboard.total_balance
  const accountValueInBaseCurrency = (account: (typeof data.accounts)[number]) => {
    if (account.currency === baseCurrencyCode) return account.balance_minor
    const totalInAccountCurrency = totalBalances.find((balance) => balance.currency === account.currency)
    if (!baseTotal.balance_minor || !totalInAccountCurrency?.balance_minor) return account.balance_minor
    return account.balance_minor * baseTotal.balance_minor / totalInAccountCurrency.balance_minor
  }
  const topAccounts = [...data.accounts]
    .sort((left, right) => accountValueInBaseCurrency(right) - accountValueInBaseCurrency(left))
    .slice(0, 3)
  const remainingAccountCount = data.accounts.length - topAccounts.length
  ...
  return <>
    ...
    <section className="mb-2.5 grid grid-cols-[1.6fr_repeat(3,1fr)] gap-2.5 max-[1100px]:grid-cols-[1.4fr_1fr_1fr] max-[820px]:grid-cols-2 max-[540px]:grid-cols-1">
      <div className="row-span-2 rounded-[15px] border border-brand bg-brand p-5 text-white shadow-[0_12px_26px_rgba(21,63,46,.14)] max-[820px]:col-span-2 max-[820px]:row-auto max-[540px]:col-auto">
        ...
        {!hasAccounts && <p className="m-0 text-[10px] text-white/55">Add an account to start building your overview.</p>}
        {hasAccounts && !data.dashboard.balance_complete && <p className="m-0 text-[10px] text-white/55">Partial total · missing {data.dashboard.unconverted_currencies.join(', ')} rate</p>}
        <div className="mt-4 grid gap-1.5 border-t border-white/12 pt-3">{hasAccounts ? <>{topAccounts.map((account, index) => <div className="grid grid-cols-[7px_1fr_auto] items-center gap-2 text-[10px]" key={account.id}><i className={`size-1.5 rounded-full ${index === 1 ? 'bg-[#ecba9e]' : index === 2 ? 'bg-[#a9a8e8]' : 'bg-lime'}`} /><span className="truncate text-white/65">{account.name}</span><b><AnimatedMoney currency={account.currency} value={account.balance_minor} /></b></div>)}{remainingAccountCount > 0 && <Link className="mt-0.5 inline-flex items-center gap-1 justify-self-start text-[9px] font-bold text-lime no-underline transition-colors hover:text-white" to={routes.accounts}>+{remainingAccountCount} more {remainingAccountCount === 1 ? 'account' : 'accounts'} <ArrowUpRight size={11} /></Link>}</> : <Link className="inline-flex items-center gap-1 justify-self-start text-[10px] font-bold text-lime no-underline" to={routes.accounts}>Add your first account <ArrowUpRight size={13} /></Link>}</div>
      </div>
